...
--ratioX: downsampling ratio for width, between 0 and 1.
--ratioY: downsampling ratio for height, between 0 and 1.
*/
...
var dir = opts.dir;
var ratioX = opts.ratioX;
var ratioY = opts.ratioY;
assert(dir, 'Must define --dir option');
assert(ratioX, 'Must define --ratioX option');
assert(ratioY, 'Must define --ratioY option');
console.log(opts);

var imagestr = fs.readFileSync(listDirPrefix + dir + '/images.txt', 'utf8');
var images = imagestr.split('\n');

function saveDownsampledAndFlipped(imageName, sourceLoc, outputLoc, outputLocFlipped, ratioX, ratioY) {
...
for (var i = 0; i < images.length; i++) {
	...
	saveDownsampledAndFlipped(images[i], sourceLoc, outputLoc, outputLocFlipped, ratioX, ratioY);
