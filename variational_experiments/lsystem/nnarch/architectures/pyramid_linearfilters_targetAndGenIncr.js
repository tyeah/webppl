...
// Eval a neural net with given bounds and output
function evalWith(net, input, bounds, output) {
	net.setBounds(bounds);
...
module.exports = NNArch.subclass(require('./localFeatures'), archname, {
	...
	downsample: NNArch.nnFunction(function(name) {
		return nn.meanpool({filterSize: 2}, name);
	}),

	constructTargetPyramid: function(inputImageTensor) {
		var pyramid = [ this.filter('target_level0_filter').eval(inputImageTensor) ];
		for (var i = 0; i < nPyramidLevels-1; i++) {
			var prev = pyramid[i];
			var ds = this.downsample('target_level'+(i+1)+'_downsample').eval(prev);
			var next = this.filter('target_level'+(i+1)+'_filter').eval(ds);
			pyramid.push(next);
		}
		return pyramid;
	},

	constructImageSoFarPyramid_complete: function(globalStore) {
		globalStore.imageSoFarPyramid = [ this.filter('gen_level0_filter').eval(globalStore.genImg.toTensor()) ];
		if (!this.training) {
			globalStore.imgSoFarDs = [];
		}
		for (var i = 0; i < nPyramidLevels-1; i++) {
			var prev = globalStore.imageSoFarPyramid[i];
			var ds = this.downsample('gen_level'+(i+1)+'_downsample').eval(prev);
			if (!this.training) {
				globalStore.imgSoFarDs.push(ds);
			}
			var next = this.filter('gen_level'+(i+1)+'_filter').eval(ds);
			globalStore.imageSoFarPyramid.push(next);
		}
	},

	constructImageSoFarPyramid_incr: function(globalStore) {
		var newPyramid = [];
		var newDs = [];
		var f = this.filter('gen_level0_filter');
		var bounds = globalStore.lastRenderedBBox;
		var out = globalStore.imageSoFarPyramid[0].clone();	// Have to copy, else we break globalStore behavior
		evalWith(f, globalStore.genImg.toTensor(), bounds, out);
		newPyramid.push(out);

		for (var i = 0; i < nPyramidLevels-1; i++) {
			var inp = out;
			var ds = this.downsample('gen_level'+(i+1)+'_downsample');
			bounds = bounds.clone();
			bounds.min.divideScalar(2).floor();
			bounds.max.divideScalar(2).ceil();
			var out = globalStore.imgSoFarDs[i].clone();
			evalWith(ds, inp, bounds, out);
			newDs.push(out);

			inp = out;
			var f = this.filter('gen_level'+(i+1)+'_filter');
			var out = globalStore.imageSoFarPyramid[i+1].clone();
			evalWith(f, inp, bounds, out);
			newPyramid.push(out);
		}

		globalStore.imageSoFarPyramid = newPyramid;
		globalStore.imgSoFarDs = newDs;
	},

	constructImageSoFarPyramid: function(globalStore) {
		// If training, always rebuild the whole pyramid (maintain derivative flow)
		if (this.training) {
			this.constructImageSoFarPyramid_complete(globalStore);
		} else {
			// If testing, do incremental updates.
			if (globalStore.imageSoFarPyramid === undefined) {
				this.constructImageSoFarPyramid_complete(globalStore);
			} else {
				this.constructImageSoFarPyramid_incr(globalStore);
			}
		}
	},
